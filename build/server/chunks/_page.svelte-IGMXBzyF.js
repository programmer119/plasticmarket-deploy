import { b as escape_html, c as attr, d as attr_class, f as ensure_array_like, k as derived } from './index.js-eXhKP-qF.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<main><section class="product-page"><div class="gallery-pane"><div class="crumb">Marketplace / ${escape_html(p().category)} / ${escape_html(p().brand)}</div><div class="product-gallery"><img${attr("src", selectedListing()?.actualPhoto || p().image)}${attr("alt", selectedListing() ? `Actual listing from ${selectedListing().sellerName || "seller"}` : p().title)}/></div><div class="product-thumbs"><button${attr_class("thumb thumb-button", void 0, { "selected": Boolean(selectedListing()) })} aria-label="Show selected listing photo"><img${attr("src", selectedListing()?.actualPhoto || p().image)} alt="Selected listing"/></button><div class="thumb"><img${attr("src", p().image)}${attr("alt", p().title)}/></div></div></div> <aside class="trade-pane"><div class="trade-sticky"><div class="product-kicker">${escape_html(p().brand)} · ${escape_html(p().series)}</div><h1>${escape_html(p().title)}</h1><div class="submeta">${escape_html(p().grade)} · ${escape_html(p().scale)} · Release ${escape_html(p().release)}</div> `);
		if (acceptedOffer()) {
			$$renderer.push("<!--[0-->");
		...
		if (selectedListing()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="trade-card"><div class="trade-label">Selected listing</div><div class="selected-seller-line"><strong>${escape_html(selectedListing().sellerName || "Seller")}${escape_html(selectedListing().demo ? " · Demo" : "")}</strong>`);
			...
			$$renderer.push(`<!--]--></div><div class="trade-price">${escape_html(krw(selectedListing().price))}</div><div class="listing-specs"><span>${escape_html(selectedListing().condition)}</span><span>Box ${escape_html(selectedListing().boxCondition)}</span><span>${escape_html(selectedListing().partsStatus)}</span><span>Manual ${escape_html(selectedListing().manualStatus)}</span></div><div class="fee">${escape_html(selectedListing().shippingMethod)} · shipping ${escape_html(krw(selectedListing().shippingCost))}</div><div class="split-actions"><a class="btn dark"${attr("href", data.user ? `/checkout?listing=${selectedListing().id}` : `/account?next=${encodeURIComponent(`/checkout?listing=${selectedListing().id}`)}`)}>Buy this listing</a><button class="btn">Make offer</button></div>`);
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<!--]--></div>`);
		} else {
			$$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte-IGMXBzyF.js.map
