import { b as escape_html, d as ensure_array_like } from './index.js-BUJDLQvZ.js';
import { t as translate } from './i18n-BCXRuS_J.js';
import { p as products } from './catalog-eJPybn3p.js';
import { P as ProductCard } from './ProductCard-0p90YFAi.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		const released = products.filter((p) => !preorder.includes(p));
		$$renderer.push(`<main><section class="market-home-head"><div class="shell market-home-row"><div><h1>${escape_html(t("marketplace"))}</h1></div><a class="btn dark" href="/browse">${escape_html(t("browse"))}</a></div></section> <section class="section"><div class="shell"><div class="section-head"><h2>${escape_html(t("new"))}</h2><a href="/browse">${escape_html(t("browse"))}</a></div><div class="product-grid"><!--[-->`);
		const each_array = ensure_array_like(released.slice(0, 4));
		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			let p = each_array[$$index];
			ProductCard($$renderer, { p });
		}
		$$renderer.push(`<!--]--></div></div></section> <section class="section"><div class="shell"><div class="section-head"><h2>${escape_html(t("preorders"))}</h2><a href="/browse">${escape_html(t("browse"))}</a></div><div class="product-grid"><!--[-->`);
		const each_array_1 = ensure_array_like(preorder);
...
//# sourceMappingURL=_page.svelte-Clc3S_cu.js.map
