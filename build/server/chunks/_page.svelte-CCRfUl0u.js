import { c as attr, b as escape_html, d as ensure_array_like, f as attr_class, k as derived } from './index.js-Chaee5kv.js';
import { k as krw } from './catalog-DCGl-LrU.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (!listing()) {
			...
			$$renderer.push(`</label></div></div><div class="panel inline-gap-top"><h2>Payment method</h2><div class="pay-options"><!--[-->`);
			...
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				...
				$$renderer.push(`<button${attr_class("payopt", void 0, { "active": method === m })}><span>${escape_html(m)}</span><span>›</span></button>`);
			}
			$$renderer.push(`<!--]--></div></div></section><aside><div class="order-box"><h2 class="flush-top">Order summary</h2><div class="line"><span>Item</span><strong>${escape_html(krw(itemAmount()))}</strong></div><div class="line"><span>${escape_html(listing().shippingMethod)}</span><strong>${escape_html(krw(listing().shippingCost))}</strong></div>`);
			if (fee() > 0) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<div class="line"><span>Marketplace fee</span><strong>${escape_html(krw(fee()))}</strong></div>`);
			} else $$renderer.push("<!--[-1-->");
			$$renderer.push(`<!--]--><div class="line total"><span>Total</span><strong>${escape_html(krw(total()))}</strong></div><button class="btn green full-gap-top"${attr("disabled", false, true)}>${escape_html("Place order")}</button>`);
			$$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte-CCRfUl0u.js.map
