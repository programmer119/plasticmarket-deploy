import { b as escape_html, c as attr, f as ensure_array_like, k as derived } from './index.js-DIZZAeri.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (lowest()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="trade-card"><div class="trade-label">Buy Now for</div><div class="trade-price">${escape_html(krw(lowest().price))}</div><div class="fee">Sold by ${escape_html(lowest().sellerName || "Seller")} · ${escape_html(lowest().shippingMethod)} · shipping ${escape_html(krw(lowest().shippingCost))}</div><div class="split-actions"><a class="btn dark"${attr("href", data.user ? `/checkout?listing=${lowest().id}` : `/account?next=${encodeURIComponent(`/checkout?listing=${lowest().id}`)}`)}>Buy now</a><button class="btn">Make offer</button></div>`);
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<!--]--></div>`);
		} else {
			$$renderer.push("<!--[-1-->");
		...
		if (data.listings.length) {
			...
			for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
				...
				$$renderer.push(`<div class="row listing-row"><div class="listing-condition"><img${attr("src", l.actualPhoto)} alt="Actual item"/><span><strong>${escape_html(l.sellerName || "Seller")}${escape_html(l.demo ? " · Demo" : "")}</strong><br/><span class="meta">${escape_html(l.condition)} · Box ${escape_html(l.boxCondition)} · ${escape_html(l.partsStatus)}</span>`);
				if (l.sellerScore) {
					$$renderer.push("<!--[0-->");
					$$renderer.push(`<br/><span class="meta">Seller ${escape_html(l.sellerScore)}% · ${escape_html(l.sellerSales || 0)} sales</span>`);
				} else $$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]--></span></div><div>${escape_html(krw(l.price))}</div><div>${escape_html(l.shippingMethod)} · ${escape_html(krw(l.shippingCost))}</div><div>${escape_html(new Date(l.createdAt).toLocaleDateString())}</div><a class="btn"${attr("href", data.user ? `/checkout?listing=${l.id}` : `/account?next=${encodeURIComponent(`/checkout?listing=${l.id}`)}`)}>Buy</a></div>`);
			}
			$$renderer.push(`<!--]--></div>`);
		} else {
			$$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte-iecWOGxF.js.map
